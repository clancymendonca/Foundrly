...
For a single index of all guides, see **[docs/README.md](../README.md)**.
...
- Feature Guides: [BADGE_SYSTEM_GUIDE.md](../features/BADGE_SYSTEM_GUIDE.md), [ENHANCED_BADGE_SYSTEM_GUIDE.md](../features/ENHANCED_BADGE_SYSTEM_GUIDE.md), [NOTIFICATION_SYSTEM_GUIDE.md](../features/NOTIFICATION_SYSTEM_GUIDE.md), [REPORTING_SYSTEM_GUIDE.md](../features/REPORTING_SYSTEM_GUIDE.md), [AI_FEATURES_GUIDE.md](../features/AI_FEATURES_GUIDE.md), [STARTUP_ANALYTICS_GUIDE.md](../features/STARTUP_ANALYTICS_GUIDE.md)
- Push/Chat: [PUSH_NOTIFICATIONS_GUIDE.md](../features/PUSH_NOTIFICATIONS_GUIDE.md), [STREAM_CHAT_GUIDE.md](../features/STREAM_CHAT_GUIDE.md)
- Troubleshooting: [TROUBLESHOOTING_GUIDE.md](../operations/TROUBLESHOOTING_GUIDE.md)
- Deployment: [DEPLOYMENT_GUIDE.md](../deployment/DEPLOYMENT_GUIDE.md), [VERCEL_DEPLOYMENT_GUIDE.md](../deployment/VERCEL_DEPLOYMENT_GUIDE.md), Docker guides (see [docs/README.md](../README.md))
...
After the repository reorganization, place new code in these locations:

| Area | Path | Notes |
|------|------|-------|
| App pages & API | `app/` | App Router only (no `pages/`). Dev/test API routes go under `app/api/dev/` |
| Components | `components/<feature>/` | Feature folders: `badges`, `chat`, `comments`, `donate`, `layout`, `moderation`, `notifications`, `profile`, `settings`, `shared`, `startup`, etc. Shared primitives stay in `components/ui/` |
| Dev-only UI | `components/dev/` | Test panels and diagnostics — not for production pages |
| Server utilities | `lib/` | Domain modules in `lib/notifications/`, `lib/badges/` (import via `@/lib/notifications/...` or `@/lib/badges/...`) |
| Hooks | `hooks/<domain>/` | e.g. `hooks/notifications/`, `hooks/badges/`, `hooks/moderation/`, `hooks/ai/` |
| Shared types | `types/` | Cross-cutting TS types (Sanity-generated types stay in `sanity/types.ts`) |
| Documentation | `docs/` | Guides grouped in `core/`, `deployment/`, `features/`, `apis/`, `operations/` |
| Scripts | `scripts/` | Maintenance and deployment scripts |

Run **`npm run verify:ci`** before opening a PR.
